...
// See Larminie & Dicks, 2003 and Laurencelle et. al., 2001
  if ((4.01e-2) - (1.4e-4)*(hp.T - 293.15))*log(v_act.i*(232/1000))>0 then
    v_act.v = N*N_m*(((4.01e-2) - (1.4e-4)*(hp.T - 293.15))*log(v_act.i*(232/1000)));
  else
    v_act.v = 0;
  end if;
// See Larminie & Dicks, 2003 and Laurencelle et. al., 2001
  if hp.T >= 312.15 then
    v_conc.v = N*N_m*(((1.1e-4) - (1.2e-6)*(hp.T - 293.15))*exp((8e-3)*(v_conc.i*(232/1000))));
  else
    v_conc.v = N*N_m*(((3.3e-3) - (8.2e-5)*(hp.T - 293.15))*exp((8e-3)*(v_conc.i*(232/1000))));
  end if;
// See Laurencelle et. al., 2001
  R_ohm.R = N*N_m*(((4.77e-4)-(3.32e-6)*(hp.T-293.15))*(1000/232));
  hp.T = 297.15;
